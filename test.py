...
for i in range(1):
	for j in range(numpts):
		# *f(xvec[j],basis,coefficents[i])*xvec[j]
		yvec[j] = f(xvec[j], basis, coefficents[i])
	temp = 0.0
	for i in range(len(yvec)-1):
		temp += ((xvec[1] + xvec[0]) / 2.0)*((yvec[2]+yvec[1])/2.0)
	print temp
	plt.plot(xvec,2.0*yvec/temp,color='blue',linewidth=2.1)
